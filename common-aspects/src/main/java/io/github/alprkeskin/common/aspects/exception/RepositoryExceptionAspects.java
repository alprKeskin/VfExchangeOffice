...
@Aspect
@Service
public class RepositoryExceptionAspects {
    @AfterThrowing(pointcut = "io.github.alprkeskin.common.aspects.util.PointcutUtil.repositoryPointcut()", throwing = "t")
    public void handleError(JoinPoint joinPoint, Throwable t) {
        if(t instanceof CustomException)
            return;
        CustomException e = new CustomException(t, "Db transaction does not properly occur.", INTERNAL_SERVER_ERROR);
